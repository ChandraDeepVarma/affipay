...
const AboutUsContent = () => {
  ...
  useEffect(() => {
    const fetchAboutContent = async () => {
      try {
        const response = await fetch("/api/site-settings/about");
        if (response.ok) {
          const data = await response.json();
          if (data.success && data.aboutUs) {
            setAboutContent(data.aboutUs);
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/site-settings/about", {
        ...
        body: JSON.stringify({ aboutUs: aboutContent }),
      });

      if (response.ok) {
        const data = await response.json();
        if (data.success) {
          toast.success(data.message || "About content updated successfully");
        } else {
          toast.error(data.message || "Failed to update about content");
        }
      } else {
            ...
            <div className="col-lg-12">
              <div className="border p-3 rounded mb-3">
                ...
                <div className="mb-20">
                  ...
                  <SunEditor
                    setContents={aboutContent}
                    onChange={(content) => setAboutContent(content)}
                    height="300px"
                    setOptions={{
                      buttonList: [
                        ["undo", "redo"],
